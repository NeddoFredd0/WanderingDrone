...
local function GetWDPlayer(inst)
    local target = FindClosestPlayerToInst(inst, 30, true)
    --print("DEBUG: nearest WD: "..target.prefab)
    return target ~= nil and target:HasTag("tapemaker") and target or nil
end
...
function PigEliteFighterBrain:OnStart()
    local root = PriorityNode(
    {
        ...
        WhileNode(function() return GetLeader(self.inst) == nil end, "Standby", --if no leader, despawn
            ParallelNode{
                StandStill(self.inst),
                LoopNode({ ActionNode(function() self.inst.components.follower:SetLeader(GetWDPlayer(self.inst)) end) }), --manually reapply leader
            }),
    }, .5)
