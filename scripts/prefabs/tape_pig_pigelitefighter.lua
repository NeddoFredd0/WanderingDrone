...
local brain = require("brains/tape_pig_pigelitefighterbrain")
...
local function onnewcombattarget(inst, data) --TODO: FIGURE OUT WHY THEY JUST LEAVE!! Maybe replace brain 
	if data ~= nil then
		--if data.target ~= nil and inst.components.follower ~= nil then --todo: remove this??
...
local function MakePigEliteFighter(variation)
    local function fn()
		...
		inst.components.timer:StartTimer("despawn_timer", TUNING.TAPE_PIG.DURATION)
    ...
    return Prefab("tape_pig_pigelitefighter"..variation, fn, assets, prefabs)
end
